...
public class TypeConverter
{
	...
	// *** NEW: Cache for the array descriptor struct type ***
	...
	public static LLVMTypeRef toLLVMType(Type type, LLVMContextRef ctx)
	{
		...
		// --- Handle ArrayType ---
		// An array variable (e.g., `int[5] arr;`) is represented by the descriptor struct { void*, i32 }.
		// The actual array data ([5 x i32]) is allocated separately.
		if (type instanceof ArrayType)
		{
			return getArrayDescStructTypeForContext(ctx);
		}
		...
		Debug.logWarning("TypeConverter encountered unknown type: " + type.getName() + ". Returning void pointer (i8*).");
	...
	{
		// --- FIX: Was LLVM.LLVMDoubleTypeInContext(ctx) ---
	...
	{
		// --- FIX: Was LLVM.LLVMDoubleTypeInContext(ctx) ---
		return LLVM.LLVMInt16TypeInContext(ctx);
	}
	...
	{
		// This one was correct, but good to confirm
	...
	public static LLVMValueRef toBoolean(LLVMValueRef value, NebulaParser.ExpressionContext ctx, LLVMContextRef moduleContext, LLVMBuilderRef builder)
	{
		...
		if (valueType == null)
		{
			Debug.logError("Cannot convert null LLVMValueRef to boolean" + (ctx != null ? " in expression: " + ctx.getText() : ""));
			return LLVMConstInt(i1Type, 0, 0); // Return false on error
		}
		...
		else
		{
			...
			Debug.logWarning("IR Warning: Conditional expression resulted in unhandled type kind: " + LLVMGetTypeKind(valueType) + (ctx != null ? " in " + ctx.getText() : ""));
